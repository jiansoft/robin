...
// Between the job will be executed only between an assigned period.
func (j *Job) Between(f time.Time, t time.Time) *Job {
    ...
    now := time.Now()
    j.fromTime = time.Date(now.Year(), now.Month(), now.Day(), f.Hour(), f.Minute(), f.Second(), f.Nanosecond(), time.Local)
    j.toTime = time.Date(now.Year(), now.Month(), now.Day(), t.Hour(), t.Minute(), t.Second(), t.Nanosecond(), time.Local)
    return j
}
...
func (j *Job) Do(fun interface{}, params ...interface{}) Disposable {
    ...
    firstInMs := (j.nextTime.UnixNano() - now.UnixNano()) / time.Millisecond.Nanoseconds()
...
func (j *Job) run() {
    adjustTime := (j.nextTime.UnixNano() - time.Now().UnixNano()) / time.Millisecond.Nanoseconds()
    if adjustTime <= 0 {
        if j.afterCalculate {
            s := time.Now()
...
func (j *Job) schedule(firstInMs int64) {
    j.Lock()
    j.taskDisposer = fiber.Schedule(firstInMs, j.run)
    j.Unlock()
}
